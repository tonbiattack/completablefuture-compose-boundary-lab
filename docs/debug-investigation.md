...
thenApply相当の処理はCompletableFuture<CompletableFuture<String>>を作る。非同期処理の結果を一つのCompletableFuture<String>へ平坦化するにはthenComposeを使う。修正前はFutureオブジェクトを文字列化し、期待する大文字文字列を返さない。 Spring Boot 3.3.5、Java 21、Mavenで再現する。

## 既存題材との差分

既存下書きのjoinによるCompletionExceptionやリトライ判定ではなく、今回は値変換と非同期段階のflattening契約を扱う。
...
`FutureService` と対応するテストを確認し、次を実行する。
...
[1]: https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CompletableFuture.html "Java公式資料"

[2]: https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/CompletionStage.html "Java公式Javadoc"
